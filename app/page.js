...
export default function Page() {
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-brand-bg text-brand-text">
        ...
        <section className="relative min-h-[90vh] flex items-center justify-center overflow-hidden border-b border-brand-accent/20 vintage-texture">
          ...
          <div className="container mx-auto px-6 text-center z-10 py-20">
            ...
            <p className="text-xl md:text-3xl mb-8 font-light text-brand-secondary tracking-[0.3em] uppercase">
              REMEDIO PROBIÓTICO
            </p>
            ...
            <button 
              ...
              className="px-10 py-4 bg-brand-primary text-brand-bg rounded-full font-bold text-lg hover:scale-105 transition-transform shadow-xl uppercase tracking-widest"
            >
              Pedir Ahora
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
              {[
                ...
                <div key={i} className="text-center group">
                  <h3 className="text-2xl font-bold mb-6 text-brand-secondary tracking-widest group-hover:text-brand-accent transition-colors">{benefit.title}</h3>
                  <div className="w-12 h-1 bg-brand-accent/20 mx-auto mb-6 group-hover:w-24 transition-all"></div>
                  <p className="text-brand-text/80 leading-relaxed text-lg">{benefit.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="container mx-auto px-6 relative z-10">
            <div className="flex flex-col md:flex-row items-center gap-20">
              ...
              <div className="md:w-1/2">
                ...
                <p className="text-2xl leading-relaxed opacity-90 font-light mb-8">
                ...
                <div className="flex items-center gap-4 text-brand-accent">
                  ...
                  <span className="uppercase tracking-[0.4em] font-bold">Hecho a Mano en el Puerto</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="py-24 bg-brand-bg" id="testimonios">
          <div className="container mx-auto px-6">
            <h2 className="text-4xl md:text-6xl text-center mb-20">Lo que dicen nuestros pacientes</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              {[
                ...
                <div key={i} className="bg-white p-12 rounded-3xl shadow-sm border border-brand-accent/10">
                  <p className="text-xl mb-8 opacity-80 leading-relaxed">{testi.text}</p>
                  <p className="font-bold text-brand-primary tracking-widest uppercase">— {testi.name}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-24 bg-white/50 border-t border-brand-accent/10">
          <div className="container mx-auto px-6 text-center">
            <h2 className="text-4xl md:text-7xl mb-12">¿Listo para tu dosis?</h2>
            <button 
              onClick={() => handleWhatsAppClick("Hola! Quiero empezar mi tratamiento con Dr. Kombucha.")}
              className="px-12 py-6 bg-brand-secondary text-brand-bg rounded-full font-bold text-xl hover:bg-brand-primary transition-colors shadow-2xl uppercase tracking-widest"
            >
              Contactar al Doctor
            </button>
          </div>
      ...
      <footer className="py-12 bg-brand-primary text-brand-bg/60 text-center text-sm tracking-widest uppercase">
